fn main() {
    ...
    println!("part 2: {:?}", part2_solution(input.as_str()));
}
    ...
    for (index, line) in input.lines().enumerate() {
        ...
        for game in games.into_iter() {
            ...
            let _ = ind
                ...
                .inspect(|x| println!("each one being passed to for: {:?}", x))
                .for_each(|v| match (v[0].parse::<u32>().unwrap(), v[1]) {
                    (x, "blue") => {
                        if x > 14 {
                            println!("blue x is: {}", x);
                            games_list[index] = 0;
...
fn part2_solution(input: &str) -> u32 {
    let mut games_list: Vec<u32> = vec![];
    for (_index, line) in input.lines().enumerate() {
        ...
        games_list.push(green_score * red_score * blue_score);
    }
    games_list.into_iter().sum()
}
...
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"#;
        assert_eq!(2286, part2_solution(input));
    }
}
